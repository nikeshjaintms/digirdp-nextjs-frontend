...
const PrivateRDP = () => {
  ...
  useEffect(() => {
    const rdplocation_plan = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/rdplocation_plan/${url_text}`
        );
  ...
  const matchedSliders = sliders.filter(
    (slider) => slider.page_id === rdplocation.id
  );
  ...
  const convertPrices = async (curr) => {
    try {
      const convertedPlans = await Promise.all(
        rdplocationplans.map(async (plan) => {
          const convertedOfferPrice = await convertCurrency(
            plan.offer_price,
            "USD",
            curr.code
            ...
            <div className="row">
              <div className="col-lg-12">
                <div className="breadcrumb-inner text-center">
                  <h1 className="title h3">{rdplocation.name}</h1>
                        ...
                        >
                          <div className="row row--15 mt_dec--40">
                            {loading
                              ...
                              : rdplocationplans.map(
                                  (rdplocationplan, index) => (
                                    <div
                                      key={index}
                                      ...
                                      <div className="pricing-header">
                                        ...
                                        <div className="pricing">
                                          ...
                                          <span className="text d-flex">
                                            ...
                                            >
                                             {parseInt(rdplocationplan.price * 12)}
                                            </span>
                                            /Year
                                          </span>
                                        </div>
